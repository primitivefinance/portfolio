...
contract TestPortfolioClaim is Setup {
    ...
    function test_revert_claim_no_position()
        public
        defaultConfig
        useActor
        isArmed
    {
        vm.expectRevert(
            abi.encodeWithSelector(
                NonExistentPosition.selector, actor(), ghost().poolId
            )
        );
        ...
        isArmed
    {
        ...
        uint128 tokensOwed = AssemblyLib.computeCheckpointDistance(
            pool.feeGrowthGlobalAsset, pos.feeGrowthAssetLast
        ).mulWadDown(pool.liquidity).safeCastTo128();

        uint256 pre = ghost().balance(actor(), ghost().asset().to_addr());
        subject().multiprocess(
            FVMLib.encodeClaim(ghost().poolId, tokensOwed, 0)
        );
        ...
        isArmed
    {
        ...
        PortfolioPool memory pool = ghost().pool();
        uint128 tokensOwed = AssemblyLib.computeCheckpointDistance(
            pool.feeGrowthGlobalQuote, pos.feeGrowthQuoteLast
        ).mulWadDown(pool.liquidity).safeCastTo128();

        subject().multiprocess(
            FVMLib.encodeClaim(ghost().poolId, 0, tokensOwed)
        );
        ...
        isArmed
    {
        ...
        (uint128 fee0, uint128 fee1) =
            (uint128(pos.tokensOwedAsset), uint128(pos.tokensOwedQuote));
        ...
        uint256 prevBalance =
            ghost().balance(actor(), ghost().asset().to_addr());
        ...
        isArmed
    {
        ...
        subject().multiprocess(
            FVMLib.encodeAllocate(
                uint8(0), ghost().poolId, uint128(startLiquidity / 5)
            )
        ); // 20%
        ...
        subject().multiprocess(
            FVMLib.encodeSwap(
                uint8(0), ghost().poolId, amountIn, amountOut, uint8(1)
            )
        ); // trade
        ...
        subject().multiprocess(
            FVMLib.encodeClaim(
                ghost().poolId, type(uint128).max, type(uint128).max
            )
        );

        uint256 evesShare = startLiquidity / 5; // 2000
        uint256 evesClaimedFees =
            ghost().balance(eve, ghost().asset().to_addr()); // 2_000 / 12_000 = ~16% of 0.00125
        ...
        assertEq(
            (evesShare * totalFeeAssetPerLiquidity) / 1 ether,
            evesClaimedFees,
            "incorrect-fee"
        );
    }

    function test_claim_succeeds()
        public
        noJit
        defaultConfig
        usePairTokens(10 ether)
        ...
        isArmed
    {
        ...
        subject().multiprocess(
            FVMLib.encodeAllocate(uint8(0), ghost().poolId, delLiquidity)
        );
        ...
        subject().multiprocess(
            FVMLib.encodeDeallocate(uint8(0), ghost().poolId, delLiquidity)
        );
