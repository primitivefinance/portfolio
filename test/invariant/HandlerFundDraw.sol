...
contract HandlerFundDraw is HandlerBase {
    function fund_asset(uint256 amount, uint256 index) public {
        ...
        deal(address(ctx.ghost().asset().to_token()), target, amount);

        uint256 preRes = ctx.ghost().reserve(address(ctx.ghost().asset().to_token()));
        uint256 preBal = ctx.ghost().balance(target, address(ctx.ghost().asset().to_token()));
        vm.prank(target);
        ...
        uint256 postBal = ctx.ghost().balance(target, address(ctx.ghost().asset().to_token()));
    ...
    function fund_quote(uint256 amount, uint256 index) public {
        ...
        deal(address(ctx.ghost().quote().to_token()), target, amount);

        uint256 preRes = ctx.ghost().reserve(address(ctx.ghost().quote().to_token()));
        uint256 preBal = ctx.ghost().balance(target, address(ctx.ghost().quote().to_token()));
        vm.prank(target);
        ...
        uint256 postBal = ctx.ghost().balance(target, address(ctx.ghost().quote().to_token()));
