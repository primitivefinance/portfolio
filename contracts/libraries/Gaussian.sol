...
function muli(
    ...
    assembly {
        ...
        if iszero(and(iszero(iszero(denominator)), or(iszero(x), eq(sdiv(z, x), y)))) {
            revert(0, 0)
        }
...
library Gaussian {
    ...
    function erfc(int256 input) internal view returns (int256 output) {
        ...
        assembly {
            ...
            {
                step := add(ERFC_F, muli(t, add(ERFC_G, muli(t, add(ERFC_H, muli(t, add(ERFC_I, muli(t, ERFC_J))))))))
            }
            {
                step := muli(
                    t,
                    add(ERFC_B, muli(t, add(ERFC_C, muli(t, add(ERFC_D, muli(t, add(ERFC_E, muli(t, step))))))))
                )
            }
    ...
    function ierfc(int256 x) internal view returns (int256 z) {
        ...
        assembly {
            ...
            r := muli(
                IERFC_A,
                sub(
                    sdiv(mul(add(IERFC_B, muli(t, IERFC_C)), ONE), add(ONE, muli(t, add(IERFC_D, muli(t, IERFC_E))))),
                    t
                )
            )
        }
        ...
        while (itr < 2) {
            ...
            assembly {
                ...
                r := add(r, sdiv(mul(err, ONE), sub(muli(IERFC_F, expWad), muli(r, err))))
